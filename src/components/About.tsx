...
import { profileContent, ProfileType } from "@/lib/profile-content";

interface AboutProps {
  currentProfile: ProfileType;
}

const About = ({ currentProfile }: AboutProps) => {
  const { ref, isInView } = useScrollReveal();

  const content = profileContent[currentProfile].about;
      ...
      <div className="container mx-auto px-4 relative z-10" ref={ref}>
        ...
        >
          <h2 className="text-5xl md:text-6xl font-bold mb-4">
            {content.title.split(' ')[0]} <span className="gradient-text">{content.title.split(' ').slice(1).join(' ')}</span>
          </h2>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          <InteractiveElement as="div" className="glass p-8 rounded-2xl">
            ...
            >
              <div>
                <div className="mb-6">
                  <p className="text-primary text-sm font-semibold mb-2">Name: <span className="text-foreground font-normal">ELMOUSATI Mohamed Ayoub</span></p>
                  <p className="text-primary text-sm font-semibold mb-2">Job Role: <span className="text-foreground font-normal">Software Engineer & AI Specialist</span></p>
                  <p className="text-primary text-sm font-semibold mb-2">Experience: <span className="text-foreground font-normal">Full-Stack & AI/ML Development</span></p>
                  <p className="text-primary text-sm font-semibold">Address: <span className="text-foreground font-normal">Morocco</span></p>
                </div>
